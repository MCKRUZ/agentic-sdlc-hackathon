...
| 13:00 | M04: Skills, Subagents & Hooks + Demo | 50 min | Explain skills/slash commands, then introduce subagents (isolated worker agents that keep your main context clean), then show hooks as demo-only (deterministic lifecycle control). Run Demo 03: build a /commit-message skill live. Show a subagent delegation. Show a PostToolUse hook in settings.json. | Watch the demo. | Building the skill live is more credible than showing a pre-built one. Go slow. For hooks, show don't build -- it's a "this exists and here's what it does" moment, not a hands-on exercise. |
| 13:50 | LAB 03: Building a Skill | 30 min | Post lab instructions. Walk the room. Participants who finish early: direct them to the subagent stretch goal in the lab instructions. | Build at least one skill for their project. Test it. Fast finishers: try the subagent delegation task. | Common issue: skills that are too generic. Help them pick something concrete from their actual workflow. |
| 14:20 | Break | 15 min | Reset demo machine for Demo 04. Check in with any participants who are struggling. | Break. | Hard stop at 14:35. |
| 14:35 | M05: MCP Servers + Demo | 30 min | Explain the Model Context Protocol in plain terms: it is a way to give the agent access to external data sources and tools beyond what's in the repository. Cover: what servers exist, how they're registered, what they cost (usually nothing extra -- compute cost is on your API usage). Run Demo 04 (see Live Demo Scripts below). | Watch the demo. Note the MCP server quick reference in the workbook. | Do not go deep on building MCP servers. That is advanced. Today's goal is using existing ones. |
| 15:05 | LAB 04: Adding an MCP Server | 25 min | Post lab instructions. Walk the room. The most common issue is install errors -- have the troubleshooting steps ready. | Install and test at least one MCP server. | context7 is the recommended default because it's well-tested and broadly useful. 25 min is enough for install + 3 prompts. |
